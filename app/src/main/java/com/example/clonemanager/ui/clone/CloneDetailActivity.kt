...
class CloneDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnAddApp.setOnClickListener {
            startActivityForResult(Intent(this, AddAppsActivity::class.java).apply {
                putExtra(EXTRA_USER_ID, userId); putExtra(EXTRA_CLONE_NAME, cloneName)
            }, REQ_ADD_APPS)
        }
        loadApps()
    }
    ...
    private fun loadApps() {
        lifecycleScope.launch {
            ...
            adapter.submit(apps)
            binding.appCount.text = "已安装应用：${apps.size}"
        }
    }

    private fun launchApp(app: AppInfo) = lifecycleScope.launch {
        showResult(withContext(Dispatchers.IO) { repo.launchPackage(userId, app.packageName) })
    }
    ...
    private fun confirmClear(app: AppInfo) {
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton("确认清除") { _, _ -> lifecycleScope.launch {
                showResult(withContext(Dispatchers.IO) { repo.clearData(userId, app.packageName) }) } }
            .show()
    }
    ...
    private fun confirmUninstall(app: AppInfo) {
        MaterialAlertDialogBuilder(this)
            ...
            .setNegativeButton("取消", null)
            .setPositiveButton("卸载") { _, _ -> lifecycleScope.launch {
                val r = withContext(Dispatchers.IO) { repo.uninstallPackage(userId, app.packageName) }
                showResult(r); if (r.success) loadApps() } }
            .show()
    }
